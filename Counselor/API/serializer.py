...
class CounselorPatientAppointmentSerializer(Serializer):
    ...
    def to_representation(self, instance):
        ...
        if  patient_email :
            selected_patients=CounselorAppointment.objects.filter(Counselor_id=selected_counselor.id).first()
        else:
            selected_patients=CustomUser.objects.filter(Counselor_id=selected_counselor.id).all()
        PatientList=[]

        PatientsData={}
        if selected_patients:
            for patient in selected_patients:
                PatientsData['first_name'] = patient.first_name
                PatientsData['last_name'] = patient.last_name
                PatientsData['email'] = patient.email
    ...
    def update(self, counselor, validated_data):
        ...
        if (AssignedToDoctor is None) and ((selected_item.first().Counselor.id==selected_counslor.id )or (selected_item.first().Counselor.id is None)):
